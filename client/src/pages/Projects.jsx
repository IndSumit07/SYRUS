...
const Projects = () => {
  ...
  const handleDeleteProject = async (id) => {
    if (
      !window.confirm(
        "Are you sure? This will delete all reports associated with this project.",
      )
    )
      return;
    try {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Manage your scraping projects and reports
          </p>
        </div>
        <button
      ...
      {projects.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <div
              key={project._id}
              className="bg-white p-6 rounded-3xl border border-gray-100 hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300 group relative"
            >
              ...
              <a
                ...
                className="text-sm text-gray-500 hover:text-orange-500 truncate block mb-4"
              >
                {project.url}
              </a>
