...
const ProjectDetails = () => {
  ...
  const handleDeleteProject = async () => {
    ...
    if (
      !window.confirm(
        "Are you sure? This will permanently delete the project and all reports.",
      )
    ) {
      return;
    }
              ...
              <div className="bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm">
                ...
                {latestReport ? (
                  <div className="space-y-6">
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold border-4 ${latestReport.score >= 80 ? "border-green-500 text-green-600 bg-green-50" : latestReport.score >= 50 ? "border-yellow-500 text-yellow-600 bg-yellow-50" : "border-red-500 text-red-600 bg-red-50"}`}
                      >
                        {latestReport.score}
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="bg-gray-50 p-4 rounded-xl">
                        ...
                        <p className="text-2xl font-bold text-gray-900">
                          {latestReport.technicalDetails?.crawlData_summary
                            ?.total_pages || 1}
                        </p>
                      </div>
                    </div>
            ...
            {reports.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-[32px] border border-gray-100">
                <p className="text-gray-500">
                  No SEO data available yet. Run a scan to generate a report.
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex flex-col items-center justify-center lg:w-1/4 space-y-4">
                      ...
                      >
                        <div className="text-center">
                          ...
                          <span className="block text-sm font-medium text-gray-500 uppercase mt-1">
                            Score
                          </span>
                        </div>
                      </div>
                      <p
                        className={`font-bold ${latestReport.score >= 80 ? "text-green-600" : latestReport.score >= 50 ? "text-yellow-600" : "text-red-600"}`}
                      >
                      ...
                      <>
                          ...
                          <div className="p-6 grid gap-6">
                            <div>
                              ...
                              <p className="text-gray-900 font-medium">
                                {latestReport.technicalDetails.seo?.title || (
                            ...
                            <div>
                              ...
                              <p className="text-gray-900 font-medium">
                                {latestReport.technicalDetails.seo
                                  ?.meta_description || (
                                  <span className="text-red-400 italic">
                                    Missing
                                  </span>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-gray-900 text-sm">
                                  {latestReport.technicalDetails.seo?.lang ||
                                    "-"}
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="p-6">
                              <div className="flex justify-between items-center mb-3">
                                ...
                                <span className="font-bold text-gray-900">
                                  {latestReport.technicalDetails.content
                                    ?.word_count || 0}
                                </span>
                              </div>
                              <div className="flex justify-between items-center mb-3">
                                ...
                                <span className="font-bold text-gray-900">
                                  {latestReport.technicalDetails.structure?.h1
                                    ?.length || 0}{" "}
                                  /{" "}
                                  {latestReport.technicalDetails.structure?.h2
                                    ?.length || 0}{" "}
                                  /{" "}
                                  {latestReport.technicalDetails.structure?.h3
                                    ?.length || 0}
                                </span>
                              </div>
                              <div className="flex justify-between items-center">
                                ...
                                <span
                                  className={`font-bold ${latestReport.technicalDetails.structure?.h1?.length === 1 ? "text-green-600" : "text-red-500"}`}
                                >
                                  {latestReport.technicalDetails.structure?.h1
                                    ?.length || 0}
                                </span>
                          ...
                          <div className="border border-gray-100 rounded-2xl overflow-hidden">
                            ...
                            <div className="p-6">
                              <div className="flex justify-between items-center mb-3">
                                ...
                                <span className="font-bold text-gray-900">
                                  {latestReport.technicalDetails.media
                                    ?.total_images || 0}
                                </span>
                              </div>
                              <div className="flex justify-between items-center mb-3">
                                ...
                                <span
                                  className={`font-bold ${latestReport.technicalDetails.media?.images_without_alt === 0 ? "text-green-600" : "text-red-500"}`}
                                >
                                  {latestReport.technicalDetails.media
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                <span className="font-bold text-gray-900">
                                  {latestReport.technicalDetails.links
                                    ?.internal_links || 0}{" "}
                                  /{" "}
                                  {latestReport.technicalDetails.links
                                    ?.external_links || 0}
                        ...
                        <div className="border border-gray-100 rounded-2xl overflow-hidden">
                          ...
                          <div className="p-6 max-h-60 overflow-y-auto custom-scrollbar">
                            {latestReport.technicalDetails.structure?.h1?.map(
                              (h, i) => (
                                <p
                                  key={`h1-${i}`}
                            ...
                            {latestReport.technicalDetails.structure?.h2?.map(
                              (h, i) => (
                                <p
                                  key={`h2-${i}`}
                                  className="text-base font-medium text-gray-700 ml-4 mb-1 display-list-item"
                                >
                                  H2: {h}
                                </p>
                              ),
                            ...
                            {latestReport.technicalDetails.structure?.h3
                              ?.length > 0 && (
                              <p className="text-sm text-gray-400 ml-8 mt-2 italic">
                                ...and{" "}
                                {
                                  latestReport.technicalDetails.structure?.h3
                                    ?.length
                        ...
                        <div className="border border-gray-100 rounded-2xl overflow-hidden">
                          <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 font-bold text-gray-700 flex justify-between items-center">
                            ...
                            <span className="text-xs bg-gray-200 text-gray-600 px-2 py-1 rounded-full">
                              {latestReport.technicalDetails.media?.all_images
                                ?.length || 0}{" "}
                              Found
                            </span>
                          </div>
                          <div className="p-0 max-h-60 overflow-y-auto custom-scrollbar divide-y divide-gray-50">
                            {latestReport.technicalDetails.media?.all_images
                              ?.length > 0 ? (
                              latestReport.technicalDetails.media.all_images.map(
                                (img, i) => (
                                  <div
                                    key={`img-${i}`}
                                  ...
                                  >
                                    <div
                                      className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${img.alt ? "bg-green-500" : "bg-red-500"}`}
                                    ></div>
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p
                                        className={`text-xs mt-0.5 ${img.alt ? "text-gray-500" : "text-red-500 font-bold"}`}
                                      >
                                        {img.alt
                                          ? `Alt: "${img.alt}"`
                                          : "Missing Alt Text"}
                              ...
                              <div className="p-6 text-center text-gray-400 text-sm">
                                No images found data available.
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="border border-gray-100 rounded-2xl overflow-hidden">
                          <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 font-bold text-gray-700 flex justify-between items-center">
                            ...
                            <span className="text-xs bg-gray-200 text-gray-600 px-2 py-1 rounded-full">
                              {latestReport.technicalDetails.links?.all_links
                                ?.length || 0}{" "}
                              Found
                            </span>
                          </div>
                          <div className="p-0 max-h-60 overflow-y-auto custom-scrollbar divide-y divide-gray-50">
                            {latestReport.technicalDetails.links?.all_links
                              ?.length > 0 ? (
                              latestReport.technicalDetails.links.all_links.map(
                                (link, i) => (
                                  <div
                                    key={`link-${i}`}
                                    className="p-3 text-sm text-blue-600 hover:underline truncate hover:bg-gray-50 px-6"
                                  >
                                    <a
                                      href={link}
                                      target="_blank"
